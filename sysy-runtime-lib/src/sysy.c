...
void putfloat(float num) {
  ...
  put_int_buffered(STDOUT_FILENO, int_part);
  put_char_buffered(STDOUT_FILENO, '.');
  // fractional part (6 decimal places, consistent with common float printing)
  float frac = num - (float)int_part;
  for (int i = 0; i < 6; i++) {
    frac *= 10.0f;
    int digit = (int)frac;
    put_char_buffered(STDOUT_FILENO, '0' + digit);
    frac -= (float)digit;
  }
}
